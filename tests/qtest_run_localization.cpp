...
void QTestRunLocalization::testGetCode() {

    RunLocalization runLocalization;

    int code = runLocalization.getCode("100:12,20");
    QVERIFY(code == 100);

    code = runLocalization.getCode("12,20");
    QVERIFY(code == -1);

    code = runLocalization.getCode("25");
    QVERIFY(code == -1);
}



void QTestRunLocalization::testGetIndices() {
    ...
    unique_ptr<vector<int>> indices = runLocalization.getIndices( data, ":" );
    ...
    indices = runLocalization.getIndices( data, ":" );
    ref_indices = { 0, 4, 20 };
    QVERIFY( equal(indices->begin(), indices->end(), ref_indices.begin()) );
...
void QTestRunLocalization::testGetControlInput() {
    ...
    unique_ptr<map<string, float>> controlInput = runLocalization.getControlInput(data, start_index, len);
    float r = (*controlInput)["r"];
    float theta = (*controlInput)["theta"];
    ...
    controlInput = runLocalization.getControlInput(data, start_index, len);
    r = (*controlInput)["r"];
    theta = (*controlInput)["theta"];
...
void QTestRunLocalization::testGetParams() {
    ...
    int start_index = (*indices)[1];
    int len = (*indices)[2] - (*indices)[1] - 1;
    unique_ptr<map<string, float>> params = runLocalization.getParams(data, start_index, len);
...
void QTestRunLocalization::testGetLandmarks() {
    ...
    unique_ptr<vector<Vector2d>> landmarks = runLocalization.getObservations(data, start_index, len);
...
void QTestRunLocalization::testGetObservations() {
...
//    unique_ptr<EKFLocalization> ekf = make_unique<EKFLocalization>( 0.1, 0.5, 0.1, 0.1 );
    ...
    unique_ptr<vector<Vector2d>> observations = runLocalization.getObservations(data, start_index, len);
